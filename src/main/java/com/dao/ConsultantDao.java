...
public class ConsultantDao {
    ...
    public boolean registerConsultant(Consultant consultant) {
        ...
        try {
            String sql = "INSERT INTO cons_dtls (fname, lname, email, password,spec_country, phone) VALUES (?, ?, ?, ?, ?, ?)";
            ...
            ps.setString(5, consultant.getSpecializedCountries());
            ps.setString(6, consultant.getPhoneNumber());
    ...
    public Consultant login(String em, String psw) {
        ...
        try {
            String sql = "select * from cons_dtls where email=? and password=?";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, em);
            ps.setString(2, psw);
            ...
            while (rs.next()) {
                ...
                u.setSpecializedCountries(rs.getString(6));
    ...
    public List<Consultant> getAllConsultant() {
        ...
        try {
            ...
            String sql = "select * from cons_dtls order by id desc";
            PreparedStatement ps = conn.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                d.setSpecializedCountries(rs.getString(6));
    ...
    public Consultant getConsultantById(int id) {
        ...
        try {
            ...
            String sql = "select * from cons_dtls where id=?";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
    ...
    public boolean updateConsultant(Consultant consultant) {
        ...
        try {
            String sql = "update cons_dtls set fname=?,lname=?,email=?,password=?,spec_country=?,phone=? where id=?";
...
public boolean deleteConsultantById(int id) {
    ...
    try {
        String sql = "DELETE FROM cons_dtls WHERE id=?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, id);
            int i = ps.executeUpdate();
